...
class CursorReader(SessionReader):
    ...
    def read_session(self, session_path: Path, workspace_path: str | None = None) -> Session:
        ...
        Args:
            session_path: Path in format "db_path#session_id"
        ...
        db_path, session_id = path_str.split("#", 1)

        conn = sqlite3.connect(db_path)
        cursor = conn.execute(
            "SELECT value FROM ItemTable WHERE key = ?", (f"workbench.panel.aichat.view.aichat.chatdata.{session_id}",)
        )
